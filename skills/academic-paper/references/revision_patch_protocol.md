...
| Apply report | same run, sidecar | `<output>.apply-report.json` — ops applied, fresh block IDs, structural flags, `preserved_ratio`, `output_draft_hash` (report format 1.1: 12-hex hash of the exact revised-draft bytes the report describes), `patch_digest` (report format 1.2: full 64-hex sha256 over the exact patch-file bytes applied — the #576 Spec B §11 content-binding that must equal the paired manifest `revision_patches[i].sha256`) |
